...
def read_data(co2_last_30_days_path, 
...
def prepare_data(df_co2_last_30_days, 
...
def merge_data(merged_df, df_outdoor_temp, data_set_ml, output_path):
    ...
    # Da der aktuelle Höchstwert in den Daten 31,2 ist und ein praktisches Minimum in Betracht gezogen wird
    median_temp = merged_data[(merged_data['ambient_temp'] >= 0) & (merged_data['ambient_temp'] <= 31.2)]['ambient_temp'].median()

    # Werte außerhalb dieses Bereichs durch den Median ersetzen
...
def logistic_regression_model(final_dataset):
    ...
    print(f'Logistische Regression: Mittlerer quadratischer Fehler: {mse}')
    print(f'Logistische Regression: R-Quadrat: {r2}')
...
def random_forest_model(final_dataset):
    ...
    # Zeitstempel in numerisches Format (Sekunden seit Epoche) umwandeln
...
def main(co2_last_30_days_path, co2_older_30_days_path, 
         temp_last_30_days_path, temp_older_30_days_path, 
         outdoor_temp_path, main_dataset_path, 
         final_dataset_path, models_directory):
